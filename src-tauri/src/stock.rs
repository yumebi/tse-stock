...
pub async fn fetch_stock(app_handle: &tauri::AppHandle, code: &str, known_name: Option<String>, need_intraday_closes: bool) -> Result<StockData, String> {
    ...
    emit("企業名取得中...");
    // -- 日本語企業名（キャッシュ → 内蔵マッピング → 無ければJPX上場銘柄一覧から動的取得） --
    let name_ja = match known_name {
        Some(name) => Some(name),
        None => lookup_company_name(app_handle, &client, code).await,
    };
...
fn japanese_name(code: &str) -> Option<String> {
    COMPANY_NAMES.iter().find(|(c, _)| *c == code).map(|(_, n)| n.to_string())
}

// ===== 内蔵マッピングに無い銘柄（新規上場等）をJPX上場銘柄一覧から動的取得 =====
const JPX_LISTED_LIST_URL: &str = "https://www.jpx.co.jp/markets/statistics-equities/misc/tvdivq0000001vg2-att/data_j.xls";

fn jpx_cache_path(app_handle: &tauri::AppHandle) -> Option<std::path::PathBuf> {
    use tauri::Manager;
    let dir = app_handle.path().app_data_dir().ok()?;
    std::fs::create_dir_all(&dir).ok()?;
    Some(dir.join("jpx_company_names.json"))
}

fn load_jpx_cache(app_handle: &tauri::AppHandle) -> std::collections::HashMap<String, String> {
    jpx_cache_path(app_handle)
        .and_then(|p| std::fs::read_to_string(p).ok())
        .and_then(|s| serde_json::from_str(&s).ok())
        .unwrap_or_default()
}

fn save_jpx_cache(app_handle: &tauri::AppHandle, map: &std::collections::HashMap<String, String>) {
    if let Some(path) = jpx_cache_path(app_handle) {
...
// 全角英数字・全角スペースを半角に変換（JPXデータは全角混在のため表示統一に必要）
...
async fn fetch_jpx_listed_names(client: &reqwest::Client) -> Option<std::collections::HashMap<String, String>> {
    use calamine::Reader;
    let bytes = client.get(JPX_LISTED_LIST_URL).send().await.ok()?.bytes().await.ok()?;
    let cursor = std::io::Cursor::new(bytes.to_vec());
    let mut workbook: calamine::Xls<_> = calamine::open_workbook_from_rs(cursor).ok()?;
    let range = workbook.worksheet_range("Sheet1").ok()?;
    let mut map = std::collections::HashMap::new();
    for row in range.rows().skip(1) {
        let code = row.get(1).map(|c| to_half_width(&c.to_string()).trim().to_string());
        let name = row.get(2).map(|c| to_half_width(&c.to_string()).trim().to_string());
...
// 日本語企業名の解決: ローカルキャッシュ（起動時に最新化される）→ 内蔵マッピング → 無ければJPXから全件取得してキャッシュ更新
async fn lookup_company_name(app_handle: &tauri::AppHandle, client: &reqwest::Client, code: &str) -> Option<String> {
    let cache = load_jpx_cache(app_handle);
    if let Some(name) = cache.get(code) {
        return Some(name.clone());
    }
    if let Some(name) = japanese_name(code) {
        return Some(name);
    }
    let fresh = fetch_jpx_listed_names(client).await?;
    let found = fresh.get(code).cloned();
    save_jpx_cache(app_handle, &fresh);
    found
}

// アプリ起動時に呼ばれる。JPX最新データとローカルキャッシュを比較し、差分があれば更新する
pub async fn update_jpx_cache_if_needed(app_handle: &tauri::AppHandle) {
    ...
    if let Some(fresh) = fetch_jpx_listed_names(&client).await {
        let current = load_jpx_cache(app_handle);
        if current != fresh {
            save_jpx_cache(app_handle, &fresh);
        }
    }
